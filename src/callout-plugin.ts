...
export default function preprocessCallouts(md: string): string {
	...
	return md.replace(re, (_full: string, rawType: string, _fold: string | undefined, title: string, bodyBlock: string) => {
		...
		return `<table style="width:100%;margin:20px 0;border-collapse:collapse;border-spacing:0"><tbody><tr><td style="border-left:3px solid ${theme.border};background:${theme.bg};padding:12px 16px;border-radius:4px">
