...
> 状态：阶段 5.1 请求续期完成，异常提示与 5.2 安全研究待推进
> 建立日期：2026-08-18
> 当前基线：v1.2.0
...
实施结果：新增 `src/preview-timeout.ts` 和 `tests/preview-timeout.test.ts`；预览服务器在每次 HTTP 请求时从新的请求时间重新计时，下一次预览和插件卸载仍清理旧服务器。服务器/端口异常提示作为后续子项保留。
