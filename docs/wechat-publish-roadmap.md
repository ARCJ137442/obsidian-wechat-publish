...
实施结果：新增 `tests/latex-input-boundaries.test.ts`，以真实 `renderMarkdownCore()` 覆盖 fenced/inline code、HTML、货币金额、转义美元符号、数字美元片段、空白边界、跨行 display 和连续 inline 公式；实现采用保护片段 + 扫描提取，未扩大到 Markdown 解析器重写。行内公式按 Obsidian 的定界符规则处理：两端贴非空白字符即可配对，成对的纯数字 `$...$` 按公式处理，未闭合的 `$100` 等货币文本保持原样，连续 `$$` 保留 display 定界符语义。另修复 Wikilink 适配层恢复代码块时对 `$'`、`$$` 等替换标记的误解释，并让含行内三反引号的 fenced code 在 LaTeX 提取前保持完整。
