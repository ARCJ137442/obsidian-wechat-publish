...
> 状态：阶段 6 CSS 基础与 CLI 评估完成；移动端发布出口计划已调查并封存，暂不进入近期实现
> 建立日期：2026-08-18
> 当前基线：v1.3.2
...
## 阶段八：移动端发布出口（P0/P1，待实现）

移动端不直接复刻桌面 Electron 工作流，而是先建立独立的“微信发布制品”：正文 HTML 片段、完整 HTML 文档、纯文本和诊断摘要共用一次渲染结果。

实施顺序：

1. 抽取发布制品，新增“保存当前笔记的微信复制版 HTML”；
2. 通过 Vault Adapter、两端共用的内联 MathJax SVG 和平台适配器消除移动路径中的 Node／Electron 调用；公式输出保持桌面现有契约，不切换到另一套渲染器；
3. 在 Android 真机评估富文本剪贴板、HTML 源码复制、文件分享和内部预览；
4. 保存 HTML 与移动安全加载通过后，才考虑删除 `isDesktopOnly`；
5. 微信官方 API、自动图片上传和移动端本地 CLI 继续排除。

完整产品语义、阶段、测试矩阵和解禁门槛见 [`Obsidian Mobile 发布出口评估与实施计划`](wechat-publish-mobile-unblock-assessment.md)。

本阶段已于 2026-08-26 调查后封存，暂不作为近期主攻。Edge Android 的扩展 Beta 只纳入限时真机 spike；在证明 HTML 能进入编辑器、正确渲染并保存草稿前，不把浏览器扩展当作稳定移动出口。
...
| 发布制品抽取与保存 HTML | 10 | 9 | 8 | 72.0 | 移动端第一优先级 |
| 移动安全加载与跨平台 Adapter | 9 | 8 | 6 | 43.2 | 保存 HTML 后立即做 |
| 移动端富文本剪贴板 | 9 | 6 | 4 | 21.6 | 先做 Android 真机 spike |
